 ...
 * @author julia
 */
public class actualizarEmpleado extends CenteredFrame {
    ...
    public actualizarEmpleado(int id, String nombre, String telefono, String correo, String fechac) {
        initComponents();
        ...
        fechactext.setText(fechac);
        this.idEmpleado = id;
    }

    public void actualizarDatoEmpleado() {
        try {
            ...
            String sql = "UPDATE empleado SET nombre = ?, telefono = ?, correo_e = ?, fecha_contratacion = ? WHERE id_em = ?";
            PreparedStatement pstmt = con.prepareStatement(sql);

            // Obtén el texto de la fecha de contratación ingresada por el usuario
            String fechaContratacionStr = fechactext.getText();

            // Valida y convierte la fecha ingresada a un formato compatible con SQL
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
            dateFormat.setLenient(false); // No permite fechas inválidas

            java.util.Date parsedDate; // Variable para almacenar la fecha convertida
            try {
                // Intenta analizar el texto ingresado como fecha en el formato correcto
                parsedDate = dateFormat.parse(fechaContratacionStr);
            } catch (ParseException e) {
                // Muestra un mensaje de error si la fecha no tiene el formato correcto
                JOptionPane.showMessageDialog(this, "La fecha debe estar en formato correcto (YYYY-MM-DD).");
                return; // Sale del método sin ejecutar la actualización
            }

            // Convierte la fecha de java.util.Date a java.sql.Date para la base de datos
            java.sql.Date sqlDate = new java.sql.Date(parsedDate.getTime());

            // Establece los parámetros en la consulta SQL preparada
            pstmt.setString(1, nombretext.getText()); // Establece el nombre del empleado
            pstmt.setString(2, telefonotext.getText()); // Establece el teléfono del empleado
            pstmt.setString(3, correotext.getText()); // Establece el correo electrónico del empleado
            pstmt.setDate(4, sqlDate); // Establece la fecha de contratación
            pstmt.setInt(5, idEmpleado); // Establece el ID del empleado a actualizar
            ...
            if (rowsUpdated > 0) {
                ...
                // Mensaje si no se encontró un empleado con el ID especificado
            ...
            JOptionPane.showMessageDialog(this, "Error al actualizar el empleado: " + e.getMessage());
        }
    }
    ...
    private void RegresarbuttActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RegresarbuttActionPerformed
        ...
        vE.setVisible(true); // Mostrar el frame de MenuGerente
